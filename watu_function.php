...
require($_SERVER['DOCUMENT_ROOT'].'/wp-content/plugins/iwatu/phpquery/phpquery.php');
...
class curl{
	...
	public function __construct(){
		...
		$this->translate=get_option('watu_translate');
		$this->options=array(
						CURLOPT_REFERER=>"www.google.com",
	...
	//function __construct() end here

	//接受请求并初始化程序的函数，也是本程序的入口
	public function get_request($url,$bugstr,$fixstr,$proxy,$x,$y,$category){
	...
	public function subjectpage($dlink){
		...
		//初始化过滤所要用到的变量
		$filter_st=get_option('watu_filter_st'); //过滤不符合数量要求的散图
		$filter_zt=get_option('watu_filter_zt'); //过滤不符合数量要求的杂图
		...
		do{
			...
			while($done=curl_multi_info_read($mh)){
				...
				if($info['http_code']==200){		//如果传输的数据不是对的，那处理也没意义吧			
					$content[$i]=curl_multi_getcontent($done['handle']);
					$doc=phpquery::newdocument($content[$i]);
					foreach (pq("img") as $li) {
						$img_link=pq($li)->attr('zoomfile');
						if($img_link!=""){
							$img_urls_output[]=$img_link;
						}
					}
					unset($content[$i]);

					if(count($img_urls_output)==0){
						echo "该主题页没有可供提取的图片连接，可能是设置了浏览权限导致的。<br />";
					}
					else{

						foreach($img_urls_output as $j=>$value){ //如果有，判断地址是否为绝对路径，倘若是相对路径则在前面加上header头
							if(!strstr($value, "http://")){
								$img_urls_output[$j]=$this->urlheader."/".$value;
							}
						}
						unset($j);
						unset($value);
						...
						if($this->bugstr!=""){
							...
							$img_urls_output=str_replace($this->bugstr,$this->fixstr,$img_urls_output);
						}
						//匹配标题
						$title=pq("title")->text();
						...
						foreach (pq("meta") as $li) {
							$keywords_temp[]=pq($li)->attr('content');
						}
						$keywords=$keywords_temp[1];

						$doc->unloadDocument();

						//按照设置里的要求过滤散图、杂图和关键词
						$img_nums=count($img_urls_output);
						if($img_nums<$filter_st){
							echo $title."：图片数量不符合要求！这个主题有".$img_nums."张图片！判断为散图。取消采集！<br />";
							unset($img_urls_output);
						}
						elseif ($img_nums>$filter_zt) {
							echo $title."：图片数量不符合要求！这个主题有".$img_nums."张图片！判断为杂图。取消采集！<br />";
							unset($img_urls_output);
						}
						elseif ($this->filter_keywords($title)) {
							echo $title."：图片内容不符合要求！标题含有被禁止采集的关键词！判断为无效主题。取消采集！<br />";
							unset($img_urls_output);
						}
						else{
						
								foreach($img_urls_output as $k=>$v){  //把最终提取出的图片链接放进数据库
									$imglinks=$v; 
		...
		do{
			...
			while($done=curl_multi_info_read($mh)){
				...
				if($info['http_code']==200){		//如果传输的数据不是对的，那处理也没意义吧
					...
					foreach (pq("h3 a") as $li) {
						$url_output[]=pq($li)->attr('href');
					}
					$doc->unloadDocument();

					if(count($url_output)!=0){
	...
	public function getimage(){	//这个函数负责下载远程web服务器上的图片
		...
		if($wpdb->num_rows==0){
			...
			echo "没有图片需要下载。即将检查是否有记录需要发表博文……<br />";
			$this->public_to_blog();
	...
	public function public_to_blog(){
		...
		foreach ($gettitles as $gettitle) {
			...
			$subjectlinks=$wpdb->get_results("select title,localpath,keywords from $this->localpath where title='{$gettitle->title}'"); //嗯，我只要分组后的title就可以检索数据了
			...
			foreach ($subjectlinks as $subjectlink) {
				...
				if($deal_title){
					...
					if(strstr($title_temp, 95)){
						...
						if($this->translate=='enable'){	//是否需要翻译？enable means yes,disable means no.
				...
				if($this->translate=='enable'){	//是否需要翻译？enable means yes,disable means no.
					...
					$keywords_temp=$subjectlink->keywords;
				}
				$keywords_temp=$this->safe($keywords_temp); //过滤一下
				
				if(strstr($keywords_temp, 44)){
					$keywords_array=array_unique(explode(",", $keywords_temp));
				}
				elseif(strstr($keywords_temp, 124)){
					...
					if($this->translate=='enable'){	//是否需要翻译？enable means yes,disable means no.
						...
						$keywords_array=array("美腿","街拍","美女","丝袜");
					}
				}
				...
				//把属于该主题的图片连接地址都放入一个数组里保存。为什么？因为后面要把它们作为附件的形式附加到所发表的博文啊~
	...
	//function baidufy end here

	//关键词过滤函数，过滤掉不想采集的主题
	public function filter_keywords($title){

		//初始化过滤关键词所要用到的变量
		$filter_keywords_temp=get_option('watu_filter_keywords'); //取出wp_options里的watu_filter_keywords来跟目标网页的keywords比较
		$filter_keywords=array_unique(explode(',', $filter_keywords_temp));

		foreach ($filter_keywords as $key => $value) {
			if(strstr($title, $value)){
				echo "发现禁止采集的关键词,被禁关键词：".$value."<br />";
				return TRUE;
				break;
			}
		}
	}
	//function filter_keywords end here
